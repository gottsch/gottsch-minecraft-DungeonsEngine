...
//	}
	 ...
	 * @param room
	 */
	public SupportedBlockProcessor(IDungeonsBlockProvider blockProvider, IDecoratedRoom room) {
	...
	public int applySupportRulesPass1(final World world, final ICoords indexCoords, 
			final ICoords worldCoords, final IArchitecturalElement elem) {
		
		int amount = 0;
		
		// NOTE every element is a consumer except AIR, NONE, 
		// if the block does not require support (ie AIR), return full amount
		if (elem == Elements.AIR || elem == Elements.SURFACE_AIR ||
				elem == Elements.NONE || elem.getBase() == Elements.SURFACE_AIR) {
			return 100;
		}
...
//				Plan.log.info("Total Amount of support coming with the HORIZONTAL(ZNeg) support: " + amount);
	 ...
	 * @return
	 */
	public int applySupportRulesPass2(final World world, final ICoords indexCoords, final ICoords worldCoords, final IArchitecturalElement elem) {
		
		int amount = 0;
		
		// NOTE every element is a consumer except AIR
		// if the block does not require support (ie AIR), return full amount
		if (elem == Elements.AIR) {
			return 100;
		}
	 ...
	 * @return
	 */
	public int getYNegSupportAmount(final World world, final ICoords indexCoords, final ICoords worldCoords, IArchitecturalElement elem) {
...
//					Plan.log.info("SupportedBlocks Matrix did not have entry - return 100.");
				...
				IArchitecturalElement supportElement = blockProvider.getArchitecturalElement(worldCoords.add(0, -1, 0), room, room.getLayout());
	 ...
	 * @return
	 */
	public int getXNegSupportAmount(final World world, final ICoords indexCoords, final ICoords worldCoords, IArchitecturalElement elem) {		
		int amount = 0;		
...
//			Dungeons2.log.info("XNeg World block " + worldBlockState.getBlock().getUnlocalizedName() + " is a supporting type block.");
			if (indexCoords.getX() == 0) {
				...
				// get the design element @ the support position
				IArchitecturalElement supportElement = blockProvider.getArchitecturalElement(worldCoords.add(-1, 0, 0), room, room.getLayout());
								
				// get the support amount from the design element that is "behind" the index coords
				if (supportElement == Elements.FACADE_SUPPORT) {
					if (elem.getBase() == Elements.FACADE) {
...
//						Dungeons2.log.debug("Getting FACADE_SUPPORT amount for NON-FACADE:" + supportElement.getBase().getHorizontalSupport());					
//						Dungeons2.log.debug("Type of element:" + elem.getName());
						amount += supportElement.getBase().getHorizontalSupport();
					}
				}
				else {
//					Dungeons2.log.debug("Getting regular support amount for block:" + supportElement.getHorizontalSupport());
	 ...
	 * @return
	 */
	public int getZNegSupportAmount(final World world, final ICoords indexCoords, final ICoords worldCoords, IArchitecturalElement elem) {		
		int amount = 0;		
		...
		if (worldBlockState != null && isSupportingBlock(world, supportPos)) {
			if (indexCoords.getZ() == 0) {
				...
				// get the design element @ the support position
				IArchitecturalElement supportElement = blockProvider.getArchitecturalElement(worldCoords.add(0, 0, -1), room, room.getLayout());
								
				// get the support amount from the design element that is "behind" the index coords
				if (supportElement == Elements.FACADE_SUPPORT && elem.getBase() != Elements.FACADE) {
	 ...
	 * @return
	 */
	public int getXPosSupportAmount(final World world, final ICoords indexCoords, final ICoords worldCoords, IArchitecturalElement elem) {		
		int amount = 0;		
		BlockPos supportPos = new BlockPos(worldCoords.getX()+1, worldCoords.getY(), worldCoords.getZ());
		IBlockState worldBlockState = world.getBlockState(supportPos);
...
//			Plan.log.info("XPos World block " + worldBlockState.getBlock().getUnlocalizedName() + " is a supporting type block.");
	 ...
	 * @return
	 */
	public int getZPosSupportAmount(final World world, final ICoords indexCoords, final ICoords worldCoords, IArchitecturalElement elem) {		
		int amount = 0;		
		BlockPos supportPos = new BlockPos(worldCoords.getX(), worldCoords.getY(), worldCoords.getZ()+1);
		IBlockState worldBlockState = world.getBlockState(supportPos);
	 ...
	 */
	public void setRoom(IDecoratedRoom room) {
		this.room = room;
	}
