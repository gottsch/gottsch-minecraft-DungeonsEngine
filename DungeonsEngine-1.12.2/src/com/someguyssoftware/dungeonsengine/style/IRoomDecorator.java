...
public interface IRoomDecorator {
	 ...
	 * @param config
	 */
	void decorate(World world, Random random, IDungeonsBlockProvider provider, IDecoratedRoom room, LevelConfig config, IDungeonsEngineConfig engineConfgi);
	...
	default public void addBlock(final World world, Random random, final IDungeonsBlockProvider provider,
			final IDecoratedRoom room, final List<Entry<IArchitecturalElement, ICoords>> zone, final IBlockState[] states, 
	 ...
	 * @return
	 */
	default public boolean hasSupport(World world, ICoords coords, IArchitecturalElement elem, Location location) {
		BlockPos pos = coords.toPos();
		IBlockState blockState = null;
		Block block = null;
		if (elem == Elements.FLOOR_AIR) {
			// check below
			blockState = world.getBlockState(pos.add(0, -1, 0));
		}
		else if (elem == Elements.CEILING_AIR) {
			// check above
			blockState = world.getBlockState(pos.add(0, 1, 0));
		}
		else if (elem == Elements.WALL_AIR) {
			switch (location) {
			case NORTH_SIDE:
	 ...
	 * @return
	 */
	default public int scaleNumForSizeOfRoom(IDecoratedRoom room, int numDecorations, LevelConfig config) {
